...
gir <- function( 
	...
	rys <- function( input, style, color ) {
		...
		title(paste("Porównanie jakości przedziałów ufności dla n =",n))
	}
	...
	gen <- function( n, jumpP, alpha, intervals ) {
		...
		if( sum( "wald" == intervals ) > 0 ) {
			...
			for(i in 0:(q/2)){
				p = jumpP * i
				densityB = dbinom(c(0:n), n, p)
				inputWald[q+1-i] = inputWald[i+1] = sum( (intervalWald[c((n+2):(2*n+2))]<p) * (intervalWald[c((2*n+3):(3*n+3))]>p) * densityB )
			}
		}
